    ...
    getTenderStatus() {
      ...
      Promise.all([defer1, defer2])
        .then(data => {
          ...
          this.$service
            .post("/PlantOpenTender/dictionaryService/GetPathById", {
              id: data[0].result.typeId
            })
            .then(res => {
              //已发布
              if (data[0].result.status == BiddingStatusCode.published) {
                ...
                //报价中
                //采购商
                if (this.auditData.companyType == 2) {
                  this.isDisabled = true;
                  this.supportText = "采购商不可投标";
                  return;
                } else if (this.auditData.companyType == 1) {
                  //供应商
                  //已报价
                  if (this.bidDetailData.tenderBidCompanyIds.indexOf(myCompanyInfo.id) != -1) {
                    this.isDisabled = true;
                    this.supportText = "已报价";
                    return;
                  } else {
                    //未报价
                    //单一来源
                    if (res.result.child.id == 1011 && this.bidDetailData.tenderBidCompanyId != myCompanyInfo.id) {
                      this.isDisabled = true;
                      this.supportText = "该项目已指定供应商";
                      return;
                    } else {
                ...
                data[0].result.status == BiddingStatusCode.cancelPublicity) 
              {
                ...
                data[0].result.status == BiddingStatusCode.publicity ||
